...
[Deploying using AWS CodePipeline](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/deploying-using-codepipeline.html)

[Codepipeline java17 image on ECR](https://gallery.ecr.aws/sam/build-java17)

[Managing permissions with AWS CloudFormation mechanisms](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/sam-permissions-cloudformation.html)
...
```bash
sam-lambda-jar$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-lambda-jar$ sam local start-api
sam-lambda-jar$ curl http://localhost:3000/
```
...
```bash
sam-lambda-jar$ sam logs -n HelloWorldFunction --stack-name sam-lambda-jar --tail
```
...
```bash
sam-lambda-jar$ cd HelloWorldFunction
...
```bash
sam delete --stack-name sam-lambda-jar
```
